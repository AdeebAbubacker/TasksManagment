...
namespace TaskManagment.Tests.Application.Features.Tasks.Queries
{
    [TestClass]
    public class GetMyTasksListQueryHandlerTests
    {
        ...
        [TestMethod]
        public async Task Handle_ValidUser_ReturnsPaginatedTasks()
        {
            // Arrange
            var userId = "user-123";
            ...
            await tasksRepository.Received(1)
                .GetUserFilter(userId, query);
        }

        [TestMethod]
        [ExpectedException(typeof(UnauthorizedAccessException))]
        public async Task Handle_MissingUserIdInToken_ThrowsUnauthorized()
        {
            // Arrange
            var httpContext = new DefaultHttpContext
            {
